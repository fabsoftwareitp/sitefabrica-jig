	...
	<div>
		...
		<main>
			...
			>
				...
				<ClientOnly>
					<ContentRenderer
						class="prose lg:prose-base prose-sm prose-slate membros-link pr-7 max-w-none"
						:value="membros"
					>
						<template #empty>
...
<script setup>
	const slug = useRoute().params.slug.toString().replace(/,/g, "/membros");
	const { data: membros } = await useAsyncData(slug, () => {
		return queryContent(slug).findOne();
	});
	const toc = computed(() => {
		if (!membros.value) return [];
		const items = membros.value.excerpt?.children;
		if (!items) return [];
		const toc = [];
